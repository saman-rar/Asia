...
interface FlipUnitProps
  extends HTMLAttributes<HTMLDivElement>, VariantProps<typeof flipUnitVariants> {
  digit: number | string
}
...
const FlipUnit: FC<FlipUnitProps> = memo(function FlipUnit({
  ...
  return (
    <div className={cn(flipUnitVariants({ size, variant }), className)}>
      ...
      <div className="absolute top-1/2 start-0 w-full h-px -translate-y-1/2 bg-background/50 z-30" />
    </div>
  )
})
...
interface FlipClockProps
  extends VariantProps<typeof flipClockVariants>, HTMLAttributes<HTMLDivElement> {
  countdown?: boolean
  targetDate?: Date
...
export default function FlipClock({
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      setTime((prev) => {
        if (prev.seconds === nextTime.seconds && prev.minutes === nextTime.minutes) {
          return prev
        }
        return nextTime
      })
